...
function extractMovieNameForSlug() {
  return slugify(els.movieName.value) || 'banner';
}

async function publishBanner(e) {
  // Deprecated: hero banners are now stored in `herobanner`.
  e.preventDefault();
  ...
  if (!movieName || !description || !watchLink || !posterFile) {
  ...
  try {
    ...
    // Insert into noticecards (same table used by the public home cards)
    // Mark as homepage hero banner so index.html shows ONLY these.
    const payload = {
...
function init() {
  ...
  els.form?.addEventListener('submit', publishBanner);
}
